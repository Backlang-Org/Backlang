...
public static class StackHelpers
{
    public static int SizeOf(IType type, TypeEnvironment environment)
    {
        ...
        if (type == environment.UInt16 || type == environment.Int16)
        {
            return 2;
        }

        if (type == environment.UInt32 || type == environment.Int32 || type == environment.Float32)
        {
            return 4;
        }

        if (type == environment.UInt64 || type == environment.Int64 || type == environment.Float64)
        {
            return 8;
        }
