...
public class ArrayExpressionImplementor : IExpressionImplementor
{
    ...
    {
        return node.Calls(CodeSymbols.Array);
    }
    ...
    {
        var value = block.AppendInstruction(Instruction.CreateConstant(new IntegerConstant(node.ArgCount),
        ...
        var arrayValuesType =
            GetOrAddArrayValueType(context.Environment.MakeArrayType(elementType, 1), context,
                out var field); //Todo: replace rank
    ...
    private static DescribedType GetOrAddArrayValueType(IType elementType, CompilerContext context,
        out DescribedField field)
    {
        var arrayValuesType = (DescribedType)context.Binder.ResolveTypes(new SimpleName(Names.ArrayValues).Qualify(""))
            .FirstOrDefault();

        if (arrayValuesType == null)
        {
            arrayValuesType =
                new DescribedType(new SimpleName(Names.ArrayValues).Qualify(""), context.Assembly) { IsStatic = true };
